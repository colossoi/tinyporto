//! Build-time preparation of the exact RGBA bytes uploaded by the renderer.
...
fn prepare_mips(
    ...
    // Build each atlas tile independently and stop before any tile disappears.
...
fn prepare_material(
    ...
    height: usize,
    columns: usize,
) -> Result<(PreparedTexture, PreparedTexture)> {
    ...
    Ok((
        prepare_mips(colors, width, height, columns, true),
        prepare_mips(normals, width, height, columns, false),
    ))
}
fn prepare_color(bytes: &[u8]) -> Result<PreparedTexture> {
...
pub fn build(repo: &Path, out: &Path) -> Result<()> {
    println!("cargo:rerun-if-changed=build/materials.rs");
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/red_brick/red_brick_diff_2k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/red_brick/red_brick_nor_gl_2k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/red_brick/red_brick_rough_2k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/rock_surface/rock_surface_diff_1k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/rock_surface/rock_surface_nor_gl_1k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/rock_surface/rock_surface_rough_1k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/rough_concrete/rough_concrete_diff_1k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/rough_concrete/rough_concrete_nor_gl_1k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/textures/rough_concrete/rough_concrete_rough_1k.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/vehicles/fiat-500/textures/rear-vent-color.png")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo.join("assets/vehicles/fiat-500/textures/engine-cover-color.png")
            .display()
    );
    ...
    let (brick_color, brick_normal) = prepare_material(
        "brick",
        [
    ...
    let (stone_color, stone_normal) = prepare_material(
        "stone",
        [
    ...
    let (mortar_color, mortar_normal) = prepare_material(
        "mortar",
        [
            include_bytes!("../../assets/textures/rough_concrete/rough_concrete_diff_1k.png"),
            include_bytes!("../../assets/textures/rough_concrete/rough_concrete_nor_gl_1k.png"),
            include_bytes!("../../assets/textures/rough_concrete/rough_concrete_rough_1k.png"),
        ],
        &whole,
        1024,
        1024,
    ...
    let textures = [
        ...
        ("STONE_NORMAL", stone_normal),
        ("MORTAR_COLOR", mortar_color),
        ("MORTAR_NORMAL", mortar_normal),
        (
            "FIAT_VENT",
            prepare_color(include_bytes!(
                "../../assets/vehicles/fiat-500/textures/rear-vent-color.png"
            ))?,
        ),
        (
            "FIAT_ENGINE",
            prepare_color(include_bytes!(
                "../../assets/vehicles/fiat-500/textures/engine-cover-color.png"
            ))?,
        ),
    ];
    let mut generated = String::from("// Generated by build/materials.rs.\n");
    for (name, texture) in textures {
        let file = format!("{name}.rgba");
        std::fs::write(out.join(&file), texture.mips.concat())?;
        let lengths: Vec<_> = texture.mips.iter().map(Vec::len).collect();
        generated.push_str(&format!(
            "pub(crate) const {name}: super::TextureData = super::TextureData {{ width: {}, height: {}, srgb: {}, mip_lengths: &{:?}, bytes: include_bytes!({file:?}) }};\n",
            texture.width, texture.height, texture.srgb, lengths));
    }
    std::fs::write(out.join("material_textures.rs"), generated)?;
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn atlas_mips_keep_neighboring_tiles_separate() {
        let texture = prepare_mips(
            vec![vec![[1.0, 0.0, 0.0, 1.0]; 4], vec![[0.0, 0.0, 1.0, 1.0]; 4]],
            2,
            2,
            2,
            true,
        ...
        assert_eq!(texture.mips[1], [255, 0, 0, 255, 0, 0, 255, 255]);
    }
}
